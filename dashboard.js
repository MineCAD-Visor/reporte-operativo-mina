...
function renderSafetyCalendar(data) {
  ...
  const todayIso = String(data.fechaActual || "").trim() || isoDate(new Date());

  const accidentA = new Set((data.accidentesMesA || []).map(String));
  const accidentB = new Set((data.accidentesMesB || []).map(String));
  const accidentC = new Set((data.accidentesMesC || []).map(String));
