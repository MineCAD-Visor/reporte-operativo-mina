...
async function loadCatalogs() {
  try {
    const response = await fetch("catalogos.json?v=1", { cache: "no-store" });
...
function addHaulRow() {
  ...
  const hasFactor = Number.isFinite(Number(equipo?.factorTon));
  const toneladas = hasFactor
    ? movimientos * Number(equipo.factorTon)
    : null;
  ...
  state.acarreoInterno.push({
    ...
    factorTon: hasFactor ? Number(equipo.factorTon) : null,
    toneladas
  });
...
function updateHaulTotals() {
  ...
  document.getElementById("totalMovedTons").textContent =
    missingFactors ? "Pendiente capacidad" : `${movedTons.toFixed(1)} t`;
  document.getElementById("totalExtraction").textContent =
    missingFactors ? "Pendiente capacidad" : `${extraction.toFixed(1)} t`;
}
